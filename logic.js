/* ==========================================
   HARDBEAT PRO - UI LOGIC (V3 - ROBUST 64 STEPS)
   ========================================== */
...
// SYNTH MEMORY
// On définit la mémoire ici pour être maître des données
...
window.addEventListener('load', () => {
    ...
    if (playBtn) {
        playBtn.onclick = () => { togglePlay(playBtn); };
    }
    console.log("UI Logic V3: Init Complete.");
});
...
function refreshGridVisuals() {
    ...
    pads.forEach((pad, i) => {
        ...
        if (realIndex >= masterLength) pad.classList.add('disabled'); else pad.classList.remove('disabled');
    ...
    accents.forEach((acc, i) => {
        ...
        if (realIndex >= masterLength) { acc.style.opacity = "0.2"; acc.style.pointerEvents = "none"; } else { acc.style.opacity = "1"; acc.style.pointerEvents = "auto"; }
...
function runDrumLoop() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        let len = masterLength; // On force la longueur maître
        let shouldPlay = true;
...
function runSynthLoop() {
    ...
    // On récupère l'état actif (TRUE/FALSE) depuis la mémoire
    const isActive2 = synthSequences.seq2[currentSynthStep];
    const isActive3 = synthSequences.seq3[currentSynthStep];

    // On passe cet état au moteur audio (le 4ème argument)
    if(window.playSynthStep) {
        window.playSynthStep(currentSynthStep, freqDataSeq2[currentSynthStep], 2, isActive2);
        if(pads3.length > 0) window.playSynthStep(currentSynthStep, freqDataSeq3[currentSynthStep], 3, isActive3);
    } 
...
function generateSmartRhythm(trackIdx) {
    ...
    for (let i = 0; i < masterLength; i++) {
        ...
        let stepInBar = i % 16; 
        switch(trackIdx) {
            case 0: if (stepInBar % 4 === 0) { if (p > 0.1) { drumSequences[trackIdx][i] = true; drumAccents[trackIdx][i] = true; } } else if (stepInBar % 2 !== 0) { if (p > 0.9) drumSequences[trackIdx][i] = true; } break;
            case 1: if (stepInBar === 4 || stepInBar === 12) { if (p > 0.05) { drumSequences[trackIdx][i] = true; drumAccents[trackIdx][i] = true; } } else if (stepInBar % 2 === 0) { if (p > 0.85) drumSequences[trackIdx][i] = true; } break;
            case 2: if (p > 0.3) drumSequences[trackIdx][i] = true; break;
            case 3: if (stepInBar === 2 || stepInBar === 6 || stepInBar === 10 || stepInBar === 14) { if (p > 0.2) drumSequences[trackIdx][i] = true; } break;
            case 4: if (p > 0.7) drumSequences[trackIdx][i] = true; break;
        }
    }
}
...
function bindControls() {
    ...
    // On utilise maintenant les objets window.xxxx pour être sûr
