/* ==========================================
   HARDBEAT PRO - UI LOGIC (FULL MASTER)
   ========================================== */
let timerDrums; let timerSynths;
let isMetroOn = false; 
let isPlaying = false;

// TOUTES LES VARIABLES D'ETAT SONT ICI
...
// Paramètres Synthés (Stockés dans l'UI maintenant)
...
function bindControls() {
    ...
    const accSlider = document.getElementById('global-accent-amount'); if(accSlider) accSlider.oninput = (e) => { globalAccentBoost = parseFloat(e.target.value); document.getElementById('accent-val-display').innerText = globalAccentBoost.toFixed(1) + 'x'; };
    ...
    const vol = document.getElementById('master-gain'); if(vol) vol.oninput = (e) => { const g = parseFloat(e.target.value); const master = new (window.AudioContext || window.webkitAudioContext)().createGain(); /* Hack to access context not needed, just volume */ }; 
    // ^ Correction: Master Gain is in core. We need a window function for it.
    // Hack: We assume window.masterGain accessible or we add a function. 
    // Let's rely on standard binding below in init.
}
...
function runSynthLoop() {
    ...
    // PLAY SYNTH (DATA FROM UI)
    if (!muteState2 && synthSequences.seq2[currentSynthStep]) {
        window.playSynthNote(freqCacheSeq2[currentSynthStep], synthVol2, 2, paramsSeq2);
    }
    if (!muteState3 && synthSequences.seq3[currentSynthStep]) {
        window.playSynthNote(freqCacheSeq3[currentSynthStep] * 0.5, synthVol3, 3, paramsSeq3);
    }
...
window.addEventListener('load', () => {
    ...
    // SAVE / CLEAR / LOAD bindings are inside bindControls
    ...
    if (playBtn) {
        playBtn.onclick = () => {
            if (isPlaying) {
                isPlaying = false; 
                clearTimeout(timerDrums); clearTimeout(timerSynths);
                playBtn.innerText = "PLAY / STOP"; playBtn.style.background = "#222"; playBtn.style.color = "#fff";
                ...
                isPlaying = true; 
                playBtn.innerText = "STOP"; playBtn.style.background = "#00f3ff"; playBtn.style.color = "#000";
                trackPositions = [0,0,0,0,0]; currentSynthStep = 0; globalTickCount = 0; synthTickCount = 0;
                runDrumLoop(); runSynthLoop();
            }
        };
    }
    console.log("UI Logic : Prêt (UI Master).");
});
