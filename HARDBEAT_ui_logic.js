/* ==========================================
   HARDBEAT PRO - UI LOGIC (MASTER CONTROLLER)
   ========================================== */
let timerDrums; let timerSynths;
let isMetroOn = false; let globalSwing = 0.06;
let isPlaying = false;

// DONNEES MAITRESSES (Définies ICI et nulle part ailleurs)
...
function savePattern(slot) {
    const data = {
        ...
        synths: {
            ...
            // On sauvegarde aussi les params sliders visuels
            params2: {
                disto: document.getElementById('synth2-disto').value,
                res: document.getElementById('synth2-res').value,
                cutoff: document.getElementById('synth2-cutoff').value,
                decay: document.getElementById('synth2-decay').value
            },
            params3: document.getElementById('synth3-disto') ? {
                disto: document.getElementById('synth3-disto').value,
                res: document.getElementById('synth3-res').value,
                cutoff: document.getElementById('synth3-cutoff').value,
                decay: document.getElementById('synth3-decay').value
            } : null
        },
        global: {
            ...
            delay: { amt: document.getElementById('global-delay-amt').value, time: document.getElementById('global-delay-time').value }
        }
    };
...
function loadPattern(slot) {
    ...
    if(data.synths.params2) {
        setSlider('synth2-disto', data.synths.params2.disto); setSlider('synth2-res', data.synths.params2.res);
        setSlider('synth2-cutoff', data.synths.params2.cutoff); setSlider('synth2-decay', data.synths.params2.decay);
    }
    ...
    if (hasSeq3Data) {
        ...
        if(data.synths.params3) {
            setSlider('synth3-disto', data.synths.params3.disto); setSlider('synth3-res', data.synths.params3.res);
            setSlider('synth3-cutoff', data.synths.params3.cutoff); setSlider('synth3-decay', data.synths.params3.decay);
        }
...
function runDrumLoop() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (shouldPlay && drumSequences[i][pos]) {
            ...
            // APPEL AUDIO VIA WINDOW (Important !)
            switch(i) { 
                case 0: window.playKick(kickSettings, acc); break; 
                case 1: window.playSnare(snareSettings, acc); break; 
                case 2: window.playHiHat(hhSettings, false, acc); break; 
                case 3: window.playHiHat(hhSettings, true, acc); break; 
                case 4: window.playDrumFM(fmSettings, acc); break; 
            }
        }
...
function runSynthLoop() {
    ...
    // CHECK SYNTH (On passe les données de l'UI vers le Core)
    if (!muteState2 && synthSequences.seq2[currentSynthStep]) window.playSynthNote(freqCacheSeq2[currentSynthStep], synthVol2, 2);
    if (!muteState3 && synthSequences.seq3[currentSynthStep]) window.playSynthNote(freqCacheSeq3[currentSynthStep] * 0.5, synthVol3, 3);
    ...
    if (e.target.classList.contains('track-btn')) { 
        ...
        if (!trackMutes[currentTrackIndex]) { 
            switch(currentTrackIndex) { 
                case 0: window.playKick(kickSettings, false); break; 
                case 1: window.playSnare(snareSettings, false); break; 
                case 2: window.playHiHat(hhSettings, false, false); break; 
                case 3: window.playHiHat(hhSettings, true, false); break; 
                case 4: window.playDrumFM(fmSettings, false); break; 
            } 
        } 
    }
});
...
window.addEventListener('load', () => {
    ...
    console.log("UI Logic : Prêt (Safe Separation).");
});
