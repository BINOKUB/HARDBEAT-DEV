/* ==========================================
   ...
   ========================================== */
// --- VARIABLES GLOBALES (Celles qui fonctionnent chez toi) ---
...
// Séquences (Définies ici pour que l'UI les contrôle)
// Note: Le fichier Audio y accède car elles sont globales par nature.
if (typeof drumSequences === 'undefined') {
    var drumSequences = Array.from({ length: 5 }, () => Array(16).fill(false));
    var drumAccents = Array.from({ length: 5 }, () => Array(16).fill(false));
    var synthSequences = { seq2: Array(16).fill(false), seq3: Array(16).fill(false) };
}

// --- SMART GENERATOR: DRUMS (Déjà validé) ---
...
// --- SMART GENERATOR: SYNTHS (NOUVEAU - AJOUT SANS RISQUE) ---
function generateSmartSynth(seqId) {
    ...
    // 2. Sélection des sliders
    const selector = (seqId === 2) ? '#grid-freq-seq2 .freq-fader' : '#grid-freq-seq3 .freq-fader';
    const faders = document.querySelectorAll(selector);
    
    if (seqId === 2) {
        // --- SEQ 2: LEAD (Gamme Mineure / Techno) ---
        const root = 440; 
        const scale = [root, root * 1.2, root * 1.5, root * 2.0]; // Fondamentale, Tierce min, Quinte, Octave
        
        // Rythme : Syncopé
        for(let i=0; i<16; i++) { 
            let chance = (i % 2 !== 0) ? 0.4 : 0.1; // Plus de chance sur les contre-temps
            if (Math.random() < chance) targetSeq[i] = true; 
        }
        
        // Mélodie : Notes de la gamme
        faders.forEach(f => { 
            const note = scale[Math.floor(Math.random() * scale.length)]; 
            f.value = Math.floor(note + (Math.random()*10)-5); // Petit detune analogique
            // On déclenche l'input pour que l'Audio et l'UI se mettent à jour
        ...
        // Rythme : Rolling (Evite le Kick)
        for(let i=0; i<16; i++) { 
            if (i % 4 === 0) { 
                if (Math.random() > 0.8) targetSeq[i] = true; // Rarement sur le Kick
            } else { 
                if (Math.random() > 0.3) targetSeq[i] = true; // Souvent ailleurs (Rumble)
            } 
        }
...
// --- FONCTIONS STANDARD (Initialisation Grille, Faders, Save/Load) ---
...
function savePattern(slot) {
    const data = {
        ...
        synths: {
            ...
            // On récupère les params via window ou sliders si besoin, ici on simplifie comme avant
            ...
            // On suppose que l'audio core gère paramsSeq2/3
...
function loadPattern(slot) {
    ...
    // Apply Settings (Code raccourci pour lisibilité, identique à avant)
    ...
    if(data.synths.params2) { setSlider('synth2-disto', data.synths.params2.disto); setSlider('synth2-res', data.synths.params2.res); setSlider('synth2-cutoff', data.synths.params2.cutoff); setSlider('synth2-decay', data.synths.params2.decay); }
    ...
    if (hasSeq3Data) {
        ...
        if(data.synths.params3) { setSlider('synth3-disto', data.synths.params3.disto); setSlider('synth3-res', data.synths.params3.res); setSlider('synth3-cutoff', data.synths.params3.cutoff); setSlider('synth3-decay', data.synths.params3.decay); }
...
function bindControls() {
    ...
    const vol = document.getElementById('master-gain'); if(vol) vol.oninput = (e) => { if (window.masterGain) window.masterGain.gain.value = parseFloat(e.target.value); };
...
function runDrumLoop() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (shouldPlay && drumSequences[i][pos]) {
            ...
            switch(i) { 
                case 0: if(window.playKick) window.playKick(acc); break; 
                case 1: if(window.playSnare) window.playSnare(acc); break; 
                case 2: if(window.playHiHat) window.playHiHat(false, acc); break; 
                case 3: if(window.playHiHat) window.playHiHat(true, acc); break; 
                case 4: if(window.playDrumFM) window.playDrumFM(acc); break; 
            }
        }
...
function runSynthLoop() {
    ...
    if(window.checkSynthTick) window.checkSynthTick(currentSynthStep);
...
document.addEventListener('click', (e) => {
    ...
    // --- ICI EST LA MODIFICATION POUR LE RND SYNTH ---
    if (e.target.classList.contains('btn-random')) { 
        const target = parseInt(e.target.dataset.target); 
        generateSmartSynth(target); // On appelle notre nouvelle fonction
        refreshGridVisuals(); 
    ...
    if (e.target.classList.contains('track-btn')) { 
        ...
        if (!trackMutes[currentTrackIndex]) { 
            switch(currentTrackIndex) { 
                case 0: if(window.playKick) window.playKick(); break; 
                case 1: if(window.playSnare) window.playSnare(); break; 
                case 2: if(window.playHiHat) window.playHiHat(false); break; 
                case 3: if(window.playHiHat) window.playHiHat(true); break; 
                case 4: if(window.playDrumFM) window.playDrumFM(); break; 
            } 
        } 
    }
});
...
window.addEventListener('load', () => {
    ...
    console.log("UI Logic : Prêt (Safe Smart Synth).");
});
