/* ==========================================
   HARDBEAT PRO - CORE AUDIO (DUAL INDEPENDENT SYNTHS)
   ========================================== */
...
let paramsSeq3 = { disto: 200, res: 8, cutoff: 2, decay: 0.4 }; // Réglages "Rumble" par défaut
...
distoNode3.curve = createDistortionCurve(200); // Un peu de drive par défaut sur le 3

// Routing: Synth2 -> Disto2 -> Master + Delay
distoNode2.connect(masterGain);
distoNode2.connect(delayNode);

// Routing: Synth3 -> Disto3 -> Master + Delay
distoNode3.connect(masterGain);
distoNode3.connect(delayNode);
...
// --- API WINDOW (UPDATES INDEPENDANTS) ---
// SEQ 2
...
// GLOBAL DELAY
window.updateDelayAmount = function(val) {
    globalDelay.amt = val;
...
window.updateDelayTime = function(val) {
    globalDelay.time = val;
...
function playKick(isAccent) {
    const osc = audioCtx.createOscillator(); const g = audioCtx.createGain(); osc.connect(g); g.connect(masterGain);
    let lvl = kickSettings.level; let decayMod = kickSettings.decay;
    if (isAccent) { lvl = Math.min(1.2, lvl * globalAccentBoost); decayMod += 0.1; }
    osc.frequency.setValueAtTime(kickSettings.pitch || 150, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + decayMod);
    g.gain.setValueAtTime(lvl, audioCtx.currentTime); g.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + decayMod); osc.start(); osc.stop(audioCtx.currentTime + decayMod);
}
function playSnare(isAccent) {
    ...
    let baseTone = snareSettings.tone || 1000; let lvl = snareSettings.level; let snap = snareSettings.snappy || 1;
...
function playHiHat(isOpen, isAccent) {
    ...
    const noise = audioCtx.createBufferSource(); noise.buffer = buffer; const filt = audioCtx.createBiquadFilter(); filt.type = 'highpass';
    let tone = hhSettings.tone || 8000; let d = isOpen ? (hhSettings.decayOpen || 0.3) : (hhSettings.decayClose || 0.05); let l = isOpen ? (hhSettings.levelOpen || 0.5) : (hhSettings.levelClose || 0.4);
...
function playDrumFM(isAccent) {
    ...
    mod.frequency.value = fmSettings.modPitch || 50; let amt = fmSettings.fmAmount || 100; let lvl = fmSettings.level || 0.5; let d = fmSettings.decay || 0.3;
    if (isAccent) { lvl = Math.min(1.0, lvl * globalAccentBoost); amt += 50; d += 0.1; }
    modG.gain.value = amt; car.frequency.value = fmSettings.carrierPitch || 100; mod.connect(modG); modG.connect(car.frequency); car.connect(mainG); mainG.connect(masterGain);
...
function playSynthNote(freq, volume, seqId) {
    ...
    // Routage vers le bon Disto Node
    ...
    vca.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + params.decay); // Decay indépendant
    
    osc.start(); osc.stop(audioCtx.currentTime + params.decay + 0.1);
}

function checkSynthTick(step) {
    // On passe l'ID 2 ou 3
    if (synthSequences.seq2[step]) playSynthNote(freqCacheSeq2[step], synthVol2, 2);
    if (synthSequences.seq3[step]) playSynthNote(freqCacheSeq3[step] * 0.5, synthVol3, 3);
}

console.log("Audio Engine : Prêt (Dual Independent Synths).");
