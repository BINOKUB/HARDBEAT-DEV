/* ==========================================
   HARDBEAT PRO - CORE AUDIO (GLOBAL STATE MASTER)
   ========================================== */
...
// --- VARIABLES GLOBALES (ATTACHÉES A WINDOW POUR EVITER LES CONFLITS) ---
// Ces variables sont la "Vérité" unique pour tout le système.
...
window.updateFreqCache = function(seqId, stepIndex, val) { 
    if (seqId === 2) window.freqCacheSeq2[stepIndex] = val; 
    if (seqId === 3) window.freqCacheSeq3[stepIndex] = val; 
};
...
function playKick(isAccent) { 
    ...
    osc.frequency.setValueAtTime(window.kickSettings.pitch || 150, audioCtx.currentTime); 
...
function playSnare(isAccent) { 
    ...
    let baseTone = window.snareSettings.tone || 1000; let lvl = window.snareSettings.level; let snap = window.snareSettings.snappy || 1; 
...
function playHiHat(isOpen, isAccent) { 
    ...
    const noise = audioCtx.createBufferSource(); noise.buffer = buffer; const filt = audioCtx.createBiquadFilter(); filt.type = 'highpass'; 
    let tone = window.hhSettings.tone || 8000; let d = isOpen ? (window.hhSettings.decayOpen || 0.3) : (window.hhSettings.decayClose || 0.05); let l = isOpen ? (window.hhSettings.levelOpen || 0.5) : (window.hhSettings.levelClose || 0.4); 
...
function playDrumFM(isAccent) { 
    ...
    mod.frequency.value = window.fmSettings.modPitch || 50; let amt = window.fmSettings.fmAmount || 100; let lvl = window.fmSettings.level || 0.5; let d = window.fmSettings.decay || 0.3; 
    if (isAccent) { lvl = Math.min(1.0, lvl * window.globalAccentBoost); amt += 50; d += 0.1; } 
    modG.gain.value = amt; car.frequency.value = window.fmSettings.carrierPitch || 100; mod.connect(modG); modG.connect(car.frequency); car.connect(mainG); mainG.connect(masterGain); 
...
function checkSynthTick(step) {
    if (!window.isMutedSeq2 && window.synthSequences.seq2[step]) playSynthNote(window.freqCacheSeq2[step], window.synthVol2, 2);
    if (!window.isMutedSeq3 && window.synthSequences.seq3[step]) playSynthNote(window.freqCacheSeq3[step] * 0.5, window.synthVol3, 3);
}

console.log("Audio Engine : Prêt (Variables Globales Chargées).");
